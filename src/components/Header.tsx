...
type NavGroup = {
  id: "practice" | "arena" | "teams";
  labelKey: "nav.practice" | "nav.arena" | "nav.forTeams";
...
const GROUPS: readonly NavGroup[] = [
  ...
  {
    id: "teams",
    labelKey: "nav.forTeams",
    routeIds: ["teams", "custom", "assessments"],
  },
];
...
export default function Header() {
  ...
  const groupActive = useCallback((group: NavGroup) => group.routeIds.some((id) => {
    const item = route(id);
    ...
    <header className="sticky top-0 z-50 border-b border-zinc-200 bg-white/90 backdrop-blur dark:border-zinc-800 dark:bg-zinc-950/90">
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-3 px-4 py-3">
        ...
        <nav aria-label="Primary" className="hidden items-center gap-1 lg:flex">
          {GROUPS.map((group, index) => (
            ...
            <div className="relative">
              ...
              >
                {t(group.labelKey, locale)} <span aria-hidden className="text-xs opacity-60">▾</span>
              </button>
              {openGroup === group.id && <DesktopMenu group={group} isActive={isActive} locale={locale} />}
            </div>
            {index === 1 && progress && <DesktopLink item={progress} active={isActive(progress.path)} locale={locale} label={t("nav.progress", locale)} />}
            </Fragment>
          ))}
        </nav>
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          <div className="hidden min-h-11 items-center gap-1 rounded-full border border-orange-200 bg-orange-50 px-3 py-1 text-xs font-semibold text-orange-700 sm:flex dark:border-orange-900 dark:bg-orange-950 dark:text-orange-300" title={t("nav.progressFull", locale)}>
      ...
      {drawerOpen && (
        <div className="fixed inset-0 z-[60] lg:hidden">
          ...
          <div ref={drawerRef} id="mobile-drawer" role="dialog" aria-modal="true" aria-label={t("nav.menu", locale)} className="absolute right-0 top-0 flex h-full w-[86%] max-w-[380px] flex-col overflow-y-auto bg-white p-4 shadow-xl dark:bg-zinc-950">
            <div className="flex items-center justify-between">
              <span className="text-sm font-bold">{t("nav.menu", locale)}</span>
              <button type="button" ref={drawerCloseRef} onClick={() => setDrawerOpen(false)} aria-label={t("nav.closeMenu", locale)} className="min-h-11 min-w-11 rounded-full border px-3 py-1.5 text-sm">✕</button>
            </div>
            <div className="mt-4 flex flex-col gap-5">
              {GROUPS.map((group, index) => (
                <Fragment key={group.id}>
                <section key={group.id} aria-labelledby={`group-${group.id}`}>
                  <h2 id={`group-${group.id}`} className="text-xs font-bold uppercase tracking-widest text-zinc-500">{t(group.labelKey, locale)}</h2>
                ...
                {index === 1 && progress && <section aria-labelledby="group-progress">
                  <h2 id="group-progress" className="text-xs font-bold uppercase tracking-widest text-zinc-500">{t("nav.progress", locale)}</h2>
                  <div className="mt-2"><MobileLink item={progress} active={isActive(progress.path)} locale={locale} label={t("nav.progress", locale)} /></div>
                </section>}
                </Fragment>
              ))}
            </div>
            <div className="mt-auto flex items-center justify-between gap-2 border-t border-zinc-200 pt-4 dark:border-zinc-800">
              ...
              <span className="text-right text-xs text-zinc-500">🔥 {streak} day streak</span>
            </div>
          </div>
        </div>
      )}
...
function MobileLink({ item, active, locale, label }: { item: RouteDefinition; active: boolean; locale: "en" | "id"; label?: string }) {
  return <Link href={item.path} aria-current={active ? "page" : undefined} className={`block min-h-11 rounded-lg px-3 py-2.5 text-sm ${active ? "bg-black text-white dark:bg-white dark:text-black" : "bg-zinc-100 dark:bg-zinc-800"}`}>{label ?? item.label[locale]}</Link>;
}
