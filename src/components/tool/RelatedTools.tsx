...
export default function RelatedTools({ route, title }: { route: RouteDefinition; title?: string }) {
  const { locale } = useLocale();
  const related = getRelatedRoutes(route).slice(0, 3);
  if (related.length === 0) return null;
  return (
    <section aria-labelledby="related-tools-title" className="mt-6">
      <h2 id="related-tools-title" className="text-xs font-bold uppercase tracking-widest text-zinc-500">{title ?? (locale === "id" ? "Lanjut berlatih" : "Keep practicing")}</h2>
